...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt = mysqli_prepare($link, $sql)) {
        mysqli_stmt_bind_param($stmt, "sssss",  $date  ,  $time    , $user_name, $action , $module );
		 if (mysqli_stmt_execute($stmt)) {
            echo "<script>alert('Accounts deleted successfully!'); window.location.href='accounts.php';</script>";
        } else {
...
<body>
    ...
    <form method="POST">
        <input type="text" name="username" placeholder="Username" value="<?php echo htmlspecialchars($accounts['username']); ?>" disabled required>
        <input type="text" name="password" placeholder="Password" value="<?php echo htmlspecialchars($accounts['password']); ?>" disabled  required>
        ...
        <input type="text" name="email" placeholder="email" value="<?php echo htmlspecialchars($accounts['email']); ?>" required disabled >
        <select name="status" required disabled >
            ...
            <option value="Monitor">Active</option>
            <option value="CPU">DISABLED</option> 
        </select>  
